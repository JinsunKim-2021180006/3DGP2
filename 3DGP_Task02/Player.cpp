...
CPlayer::~CPlayer()
{
	...
	if (m_pBulletsShader) m_pBulletsShader->Release();
}
...
CSpaceshipPlayer::CSpaceshipPlayer(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext)
{
	m_pCamera = ChangeCamera(/*SPACESHIP_CAMERA*/THIRD_PERSON_CAMERA, 0.0f);
	...
	for (int i = 0; i < BULLETS; ++i) {
		CGameObject* pBulletMesh = CGameObject::LoadGeometryFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, "Model/Mi24.bin", m_pBulletsShader);
		pBulletObject = new CBulletObject(m_fBulletEffectiveRange);
		pBulletObject->SetChild(pBulletMesh);
...
void CSpaceshipPlayer::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
...
CCamera* CSpaceshipPlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	DWORD nCurrentCameraMode = (m_pCamera) ? m_pCamera->GetMode() : 0x00;
	if (nCurrentCameraMode == nNewCameraMode) return(m_pCamera);
	switch (nNewCameraMode)
...
void CSpaceshipPlayer::OnPrepareRender()
{
	CPlayer::OnPrepareRender();
}
...
//터레인 충돌 확인을 위한 코드
void CSpaceshipPlayer::OnPlayerUpdateCallback(float fTimeElapsed)
{
...
void CSpaceshipPlayer::OnCameraUpdateCallback(float fTimeElapsed)
{
...
void CSpaceshipPlayer::FireBullet(CGameObject* pLockedObject)
{
	if (pLockedObject)
	{
