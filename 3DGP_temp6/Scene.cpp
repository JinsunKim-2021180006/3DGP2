...
void CScene::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	CreateCbvSrvDescriptorHeaps(pd3dDevice, 0, 4 + 4 + 1 + 3 + 3 + 2 + 50 + 4 ); //Enemy(4), Player(4), Skybox(1), Terrain(3), Water(3), glass(2), Bullet(50), reflection(4)

	BuildDefaultLightsAndMaterials();

	m_pSkyBox = new CSkyBox(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature);

	XMFLOAT3 xmf3Scale(35.0f, 15.0f, 35.0f);
	XMFLOAT4 xmf4Color(0.0f, 1.0f, 1.0f, 0.0f);

#ifdef _WITH_TERRAIN_PARTITION
	m_pTerrain = new CHeightMapTerrain(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature, _T("Terrain/Mytarrain.raw"), 257, 257, 13, 13, xmf3Scale, xmf4Color);
#else
	m_pTerrain = new CHeightMapTerrain(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature, _T("Terrain/Mytarrain.raw"), 257, 257, 257, 257, xmf3Scale, xmf4Color);
#endif

	m_pTerrainWater = new CTerrainWater(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature, 257 * xmf3Scale.x, 257 * xmf3Scale.z);
	m_pTerrainWater->SetPosition(+(257 * xmf3Scale.x * 0.5f), 140.0f, +(257 * xmf3Scale.z * 0.5f));
...
ID3D12RootSignature *CScene::CreateGraphicsRootSignature(ID3D12Device *pd3dDevice)
{
	...
	pd3dDescriptorRanges[6].BaseShaderRegister = 14; //t14~16: gtxtTerrainTexture
	pd3dDescriptorRanges[6].RegisterSpace = 0;
	pd3dDescriptorRanges[6].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[7].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[7].NumDescriptors = 3;
	pd3dDescriptorRanges[7].BaseShaderRegister = 17; //t17: gtxtWaterBaseTexture, t18: gtxtWaterDetailTexture, t19: gtxtWaterDetailAlphaTexture
	pd3dDescriptorRanges[7].RegisterSpace = 0;
	pd3dDescriptorRanges[7].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[8].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[8].NumDescriptors = 1;
	pd3dDescriptorRanges[8].BaseShaderRegister = 20; //t20: gtxtTexture
	pd3dDescriptorRanges[8].RegisterSpace = 0;
	pd3dDescriptorRanges[8].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[9].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[9].NumDescriptors = 1;
	pd3dDescriptorRanges[9].BaseShaderRegister = 21; //gtxtTextureCube
	...
	pd3dRootParameters[8].DescriptorTable.pDescriptorRanges = &(pd3dDescriptorRanges[5]);	//skybox
	pd3dRootParameters[8].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;

	pd3dRootParameters[9].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	pd3dRootParameters[9].DescriptorTable.NumDescriptorRanges = 1;
	pd3dRootParameters[9].DescriptorTable.pDescriptorRanges = &(pd3dDescriptorRanges[6]);	//tarrain
	pd3dRootParameters[9].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;

	pd3dRootParameters[10].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	pd3dRootParameters[10].DescriptorTable.NumDescriptorRanges = 1;
	pd3dRootParameters[10].DescriptorTable.pDescriptorRanges = &pd3dDescriptorRanges[7];	//water
	pd3dRootParameters[10].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;

	pd3dRootParameters[11].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV; 
	pd3dRootParameters[11].Descriptor.ShaderRegister = 3;	//Texture Animation (4x4) Matrix
	...
	pd3dRootParameters[12].DescriptorTable.pDescriptorRanges = &(pd3dDescriptorRanges[8]);	//billbord
	...
	pd3dRootParameters[14].DescriptorTable.pDescriptorRanges = &pd3dDescriptorRanges[9]; //gtxtTextureCube
...
void CScene::CreateShaderVariables(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	UINT ncbWaterBytes = ((sizeof(MATERIALS) + 255) & ~255); //256의 배수
	m_pd3dTerrainWater = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbWaterBytes, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);

	m_pd3dTerrainWater->Map(0, NULL, (void**)&m_WaterAnmation);
}
...
void CScene::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	::memcpy(&m_WaterAnmation->xmf4x4WaterAnimation, &xmf4x4WaterAnimation, sizeof(XMFLOAT4X4));

	D3D12_GPU_VIRTUAL_ADDRESS WaterAnimationAdd = m_pd3dTerrainWater->GetGPUVirtualAddress();
	pd3dCommandList->SetGraphicsRootConstantBufferView(11, WaterAnimationAdd);
...
void CScene::ReleaseShaderVariables()
{
	...
	if (m_pd3dTerrainWater)
	{
		m_pd3dTerrainWater->Unmap(0, NULL);
		m_pd3dTerrainWater->Release();
	}
...
bool CScene::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		switch (wParam)
		{
		case 'W': m_pPlayer->Move(0x01, 5.f, false); break;
		case 'S': m_pPlayer->Move(0x02, 5.f, false); break;
		case 'A': m_pPlayer->Move(0x04, 5.f, false); break;
		case 'D': m_pPlayer->Move(0x08, 5.f, false); break;
		case 'Q': m_pPlayer->Move(0x10, 5.f, false); break;	//UP
		case 'E': m_pPlayer->Move(0x20, 5.f, false); break;	//DOWN
		default:
			break;
		}
		break;
